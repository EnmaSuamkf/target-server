...
test("E2E smoke: remote workflow with 2 steps + start → local execution + server events", async () => {
	resetSyncExecutorState();
	const hubCfg = loadConfig();
	const cookie = await login(base);

	// Client registers on first tick.
	...
	// Two steps + start (sequenced — one command per poll cycle).
